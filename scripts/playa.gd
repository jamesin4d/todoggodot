...
var look_vector := Vector3() # this is calculated runtime 
...
#-------- interaction and pick up variables
...
func interact_process():
	if interact_ray.is_colliding():
		var x = interact_ray.get_collider()
		if x.has_method("pick_up"):
			$HUD/Interact_text.set_text("[F] pick up: " + x.get_name())
		elif x.has_method("interact"):
			$HUD/Interact_text.set_text("[E] interact with: " + x.get_name())
		else:
			$HUD/Interact_text.set_text("")
	var d = (look_pos.get_global_transform().origin - cam.get_global_transform().origin).normalized()
...
#handles the throwing input gathering 
func throwing(delta):
	if carried_object != null:
...
# take direction from move_axis changes based on input in movement_input 		
...
#take in movement related input
func movement_input():
